...
const AllRooms = () => {
  ...
  console.log('AllRooms - currency:', currency);
  ...
  const matchesPriceRange = (room) => {
    return selectedFilters.priceRanges.length === 0 || selectedFilters.priceRanges.
    some((range) => {
  ...
  const filteredRooms = useMemo(() => {
    ...
    let filtered = rooms.filter(room => {
      ...
      // Check room type
      const matchesType = selectedFilters.roomTypes.length === 0 || selectedFilters.roomTypes.includes(room.roomType);
      
      // Check price range
      const matchesPrice = selectedFilters.priceRanges.length === 0 || selectedFilters.priceRanges.some((range) => {
        const [min, max] = range.split(' to ').map(Number);
        return room.pricePerNight >= min && room.pricePerNight <= max;
      });
      ...
      return matchesType && matchesPrice && matchesDestination;
    });
  ...
  console.log('AllRooms - filteredRooms:', filteredRooms);
        ...
        <div className='px-5 pt-5'>
            ...
            {roomTypes.map((room, index) => ( 
                <CheckBox key={index} label={room} selected={selectedFilters.
                  roomTypes.includes(room)} onChange={(checked) => handleFilterChange(checked, room, 'roomTypes')} />
        ))}
        ...
        <div className='px-5 pt-5 pb-7'>
          ...
          {sortOptions.map((option, index) => (
            <RadioButton key={index} label={option} selected = {
              selectedSort === option} onChange={() => handleSortChange(option)} />
          ))}
        </div>
